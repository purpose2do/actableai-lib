...
class SplitDateTimeCrossValidation(IChecker):
    def __init__(self, level, name="SplitDateTimeCrossValidation"):
        self.name = name
        self.level = level

    def check(self, split_datetime: bool, kfolds: int) -> Optional[CheckResult]:
        """Check if split datetime is enabled and kfolds is greater than 1.
        ...
        Returns:
            Optional[CheckResult]: Check result.
        """
        if split_datetime and kfolds > 1:
            return CheckResult(
                name=self.name,
                level=self.level,
...
class SplitDateTimeColumnExist(IChecker):
    def __init__(self, level, name="SplitDateTimeColumnExist"):
        self.name = name
        self.level = level

    def check(
        self, split_datetime: bool, date_time_column: Optional[str]
    ) -> Optional[CheckResult]:
        """Check if split datetime is enabled and date_time_column is not None.
        ...
        Returns:
            Optional[CheckResult]: Check result.
        """
        if split_datetime and date_time_column is None:
            return CheckResult(
                name=self.name,
                level=self.level,
