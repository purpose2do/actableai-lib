...
def _paths_to_leaves(tree, shorten_conditions=True):
    def _walk(tree, result, node_id=0, depth=0, cond=[]):
        ...
        if is_split_node:
            _walk(tree, result, left_node, depth + 1, cond + [(tree.feature[node_id], "<=", tree.threshold[node_id])])
            _walk(tree, result, right_node, depth + 1, cond + [(tree.feature[node_id], ">", tree.threshold[node_id])])
        else:
            result.append({
                "conditions": _shorten_conditions(cond) if shorten_conditions else cond,
                "values": tree.value[node_id],
                "class": np.argmax(tree.value[node_id]),
                "precision": np.max(tree.value[node_id]) * 1 / np.sum(tree.value[node_id])
            })
...
def generate_cluster_descriptions(tree, features, dummy_columns=None):
    ...
    def _path_iter():
        ...
        for path in paths:
            description = "{:.2f}% of samples that satisfy ".format(path["precision"] * 100)
            for i, cond in enumerate(path["conditions"]):
                ...
                if dummy_columns is not None and features[fid] in dummy_columns:
                    ...
                    value = tokens[-1]
                    description += "{} {} {}".format(name, "is" if op == ">" else "is not", value)
                else:
                    description += features[fid] + " " + op + " " + (
                        "{%.2f}".format(threshold) if type(threshold) is float else str(threshold))
