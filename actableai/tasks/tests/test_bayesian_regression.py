...
class TestBayesianRegression:
    ...
    def test_full_data_multivariate_result(self, df, task):
        re = task.run(df, features=["x", "y", "w"], target="z", trials=5, polynomial_degree=2)
        # print(re)
        assert re["status"] == "SUCCESS"
        assert "validation_table" in re["data"]
        assert "prediction_table" in re["data"]
        ...
        assert [x["name"] for x in re["data"]["coeffs"]["univariate"]] == ['x', 'y', 'w_a', 'w_b']
        assert [x["name"] for x in re["data"]["coeffs"]["multivariate"]] == ['x', 'y', 'w_a', 'w_b', 'x^2',\
            'x y', 'x w_a', 'x w_b', 'y^2', 'y w_a', 'y w_b']
    ...
    def test_polynomial_degree(self, df, task):
        re = task.run(df, features=["x", "y"], target="z", polynomial_degree=3, trials=5)
        assert re["status"] == "SUCCESS"
        assert "validation_table" in re["data"]
        assert "prediction_table" in re["data"]
        assert "evaluation" in re["data"]
    ...
    def test_priors(self, df, task):
        ...
        re = task.run(df, features=["x", "y", "w"], target="z", polynomial_degree=3, trials=5, priors=priors)
        assert re["status"] == "SUCCESS"
