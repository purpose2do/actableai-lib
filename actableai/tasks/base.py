...
class AAITask(ABC):
    ...
    def __init__(self,
        ...
        self.optimize_memory_allocation_nrmse_threshold = optimize_memory_allocation_nrmse_threshold
        ...
        self.optimize_gpu_memory_allocation_nrmse_threshold = optimize_gpu_memory_allocation_nrmse_threshold
        ...
        if self.optimize_memory_allocation and not self.use_ray:
            ...
            logging.warning("`optimize_memory_allocation` is set to False: `use_ray` is False")
        if self.collect_memory_usage and not self.use_ray:
            self.collect_memory_usage = False
            logging.warning("`collect_memory_usage` is set to False: `use_ray` is False")
        if self.optimize_gpu_memory_allocation and not self.use_ray:
            self.optimize_gpu_memory_allocation = False
            logging.warning("`optimize_gpu_memory_allocation` is set to False: `use_ray` is False")
        if self.collect_gpu_memory_usage and not self.use_ray:
            self.collect_gpu_memory_usage = False
            logging.warning("`collect_gpu_memory_usage` is set to False: `use_ray` is False")

        if self.optimize_memory_allocation and self.resources_predictors_actor is None:
            self.optimize_memory_allocation = False
            logging.warning("`optimize_memory_allocation` is set to False: `resources_predictors_actor` is None")
        if self.collect_memory_usage and self.resources_predictors_actor is None:
            self.collect_memory_usage = False
            logging.warning("`collect_memory_usage` is set to False: `resources_predictors_actor` is None")
        if self.optimize_gpu_memory_allocation and self.resources_predictors_actor is None:
            self.optimize_gpu_memory_allocation = False
            logging.warning("`optimize_gpu_memory_allocation` is set to False: `resources_predictors_actor` is None")
        if self.collect_gpu_memory_usage and self.resources_predictors_actor is None:
            self.collect_gpu_memory_usage = False
            logging.warning("`collect_gpu_memory_usage` is set to False: `resources_predictors_actor` is None")

    def _predict_resource(self,
                          resource_predicted: ResourcePredictorType,
                          task: TaskType,
                          features: dict) -> Tuple[Optional[float], Optional[float]]:
        """
        ...
        """
        model_metrics = ray.get(self.resources_predictors_actor.get_model_metrics.remote(
            resource_predicted,
            task
        ))
        ...
        if model_metrics["NRMSE"] < nrmse_threshold:
            predicted_resource = ray.get(self.resources_predictors_actor.predict.remote(
                resource_predicted,
                task,
                features
            ))
        ...
        def decorator(function: Callable) -> Callable:
            ...
            def _func(task_object, *args, **kwargs):
                ...
                if gpus is not None:
                    try:
                        ...
                        logging.warning(f"Visible devices must be set before GPUs have been initialized ({e})")
                ...
                if task_object.use_ray:
                    ...
                    if num_cpus is not None:
                        ...
                        cpu_percent = psutil.cpu_percent(interval=task_object.cpu_percent_interval, percpu=True)

                        new_cpu_affinity = np.array(cpu_percent).argsort()[:num_cpus].tolist()
            ...
            def wrapper(task_object: AAITask, *args, **kwargs):
                ...
                if task_object.use_ray:
                    ...
                    max_memory_prediction_features, max_memory_prediction_features_full = None, None
                    max_gpu_memory_prediction_features, max_gpu_memory_prediction_features_full = None, None
                    predicted_max_memory_raw = None
                    predicted_max_gpu_memory_raw = None

                    # Extract features from the function arguments
                    if task_object.optimize_memory_allocation or task_object.collect_memory_usage:
                        max_memory_prediction_features, max_memory_prediction_features_full = extract_features(
                    ...
                    if task_object.optimize_gpu_memory_allocation or task_object.collect_gpu_memory_usage:
                        max_gpu_memory_prediction_features, max_gpu_memory_prediction_features_full = extract_features(
                    ...
                    if task_object.optimize_memory_allocation:
                        predicted_max_memory_raw, predicted_max_memory = task_object._predict_resource(
                            ResourcePredictorType.MAX_MEMORY,
                            task,
                            max_memory_prediction_features
                        )

                        if predicted_max_memory is not None and predicted_max_memory > 0:
                            ray_params["memory"] = int(predicted_max_memory)

                    # Optimize GPU memory allocation
                    if task_object.optimize_gpu_memory_allocation:
                        predicted_max_gpu_memory_raw, predicted_max_gpu_memory = task_object._predict_resource(
                            ResourcePredictorType.MAX_GPU_MEMORY,
                            task,
                            max_gpu_memory_prediction_features
                        )

                        if predicted_max_gpu_memory is not None and predicted_max_gpu_memory > 0:
                            if "resources" not in ray_params:
                                ray_params["resources"] = {}
                            ray_params["resources"]["gpu_memory"] = int(predicted_max_gpu_memory)
                    ...
                    if task_object.collect_memory_usage:
                        resource_to_profile |= ResourceProfilerType.RSS_MEMORY | ResourceProfilerType.SWAP_MEMORY
                    ...
                    if task_object.collect_memory_usage or task_object.collect_gpu_memory_usage:
                        actor_handle = ray.remote(FunctionWrapperActor).options(**ray_params).remote()
                        profiling_results, data = ray.get(actor_handle.run.options(name=task).remote(
                            profile_function, resource_to_profile, True, _func, task_object, *args, **kwargs
                        ))

                        # If the task succeeded add the collected data to the respective models
                        if type(data) is not dict or data.get("status", "SUCCESS") == "SUCCESS":
                            if task_object.collect_memory_usage:
                                max_memory = int(profiling_results.get_max_profiled(
                                    ResourceProfilerType.RSS_MEMORY | ResourceProfilerType.SWAP_MEMORY
                                ))

                                logging.info(f"Collect max memory usage for task: {task}")
                                logging.info(f"    predicted: {predicted_max_memory_raw}")
                            ...
                            if task_object.collect_gpu_memory_usage:
                                max_gpu_memory = int(profiling_results.get_max_profiled(
                                    ResourceProfilerType.GPU_MEMORY
                                ))

                                logging.info(f"Collect max GPU memory usage for task: {task}")
                                logging.info(f"    predicted: {predicted_max_gpu_memory_raw}")
                        ...
                        actor_handle = ray.remote(FunctionWrapperActor).options(**ray_params).remote()
                        data = ray.get(actor_handle.run.options(name=task).remote(_func, task_object, *args, **kwargs))
