...
class AverageEnsembleClassifier:
    ...
    def predict(self, X):
        ...
        if len(self.class_labels) == 2:
            pred_labels = [
                self.class_labels[0]
                if x < 0.5 else self.class_labels[1]
                for x in pred_probas
            ]
        else:
            pred_labels = [
                self.class_labels[np.argmax(x)]
                for x in pred_probas
            ]
...
def run_cross_validation(classification_train_task,
                         problem_type,
                         positive_label,
                         presets,
                         hyperparameters,
    ...
    for kfold_index, (predictor, important_features, evaluate, df_val_pred_prob, leaderboard) in enumerate(cross_val_results):
        ...
        df_val_cross_val_pred_prob.append(json.loads(df_val_pred_prob.to_json(orient="table"))["data"])
    ...
    evaluate = {
        ...
        "confusion_matrix": np.mean(cross_val_evaluates["confusion_matrix"], axis=0).tolist(),
        "confusion_matrix_std_err": (np.std(cross_val_evaluates["confusion_matrix"], axis=0) / sqrt_k).tolist()
    }

    if evaluate["problem_type"] == "binary":
        auc_curve = {
            "False Positive Rate": np.mean(cross_val_auc_curves["False Positive Rate"], axis=0).tolist(),
            "False Positive Rate std err": (
                    np.std(cross_val_auc_curves["False Positive Rate"], axis=0) / sqrt_k).tolist(),
            "True Positive Rate": np.mean(cross_val_auc_curves["True Positive Rate"], axis=0).tolist(),
            "True Positive Rate std err": (
                    np.std(cross_val_auc_curves["True Positive Rate"], axis=0) / sqrt_k).tolist(),
            "thresholds": np.mean(cross_val_auc_curves["thresholds"], axis=0).tolist(),
            "thresholds_std_err": (np.std(cross_val_auc_curves["thresholds"], axis=0) / sqrt_k).tolist(),
    ...
    return ensemble_model, important_features, evaluate, df_val_cross_val_pred_prob, df_val, leaderboard
