...
def call_dataset_generator(tmp_path: Path,
    ...
    df = DatasetGenerator.generate(columns_parameters=columns_parameters,
                                   rows=rows,
    ...
    if save_parameters_path is not None:
        df = None
        df = DatasetGenerator.generate_from_file(parameters_path=save_parameters_path,
...
class TestGenerateDataset:
    ...
    def test_generate_and_save_simple_dataset(self, tmp_path):
        ...
        output_path = tmp_path / "dataset.csv"
        res = DatasetGenerator.generate(columns_parameters=columns_parameters,
                                        rows=10,
                                        output_path=output_path)
...
class TestGenerateDatasetParameters:
    ...
    def test_rows(self, tmp_path, save_output, save_parameters, rows):
        ...
        df = call_dataset_generator(tmp_path, columns_parameters, rows, save_output, save_parameters)

        assert len(df) == rows

    @pytest.mark.parametrize("columns", [2, 4, 8])
    @pytest.mark.parametrize("column_type", ["text", "number", "date"])
    def test_columns(self, tmp_path, save_output, save_parameters, columns, column_type):
        columns_parameters = [
            {
                "type": column_type
            }
        ...
        df = call_dataset_generator(tmp_path, columns_parameters, 10, save_output, save_parameters)

        assert len(df.columns) == columns

    @pytest.mark.parametrize("columns", [2, 4, 8])
    @pytest.mark.parametrize("random_state", [0, 1, 2])
    def test_columns_mixed_type(self, tmp_path, save_output, save_parameters, columns, random_state):
    ...
    def test_no_type(self, tmp_path, save_output, save_parameters):
        ...
        with pytest.raises(Exception):
            call_dataset_generator(tmp_path, columns_parameters, 10, save_output, save_parameters)

    @pytest.mark.parametrize("random_state", [0, 1, 2])
    @pytest.mark.parametrize("column_type", ["text", "number", "date"])
    def test_random_state_one_column(self, tmp_path, save_output, save_parameters, random_state, column_type):
        columns_parameters = [
            {
                "type": column_type
            }
        ]
        df_1 = call_dataset_generator(tmp_path, columns_parameters, 10, save_output, save_parameters, random_state)
        df_2 = call_dataset_generator(tmp_path, columns_parameters, 10, save_output, save_parameters, random_state)
        df_3 = call_dataset_generator(tmp_path, columns_parameters, 10, save_output, save_parameters)

        assert df_1.equals(df_2)
        assert not df_1.equals(df_3)

    @pytest.mark.parametrize("random_state", [0, 1, 2])
    def test_random_state_three_columns(self, tmp_path, save_output, save_parameters, random_state):
        columns_parameters = [
            {
                "type": "text"
            },
    ...
    @pytest.mark.parametrize("column_type", ["text", "number", "date"])
    def test_no_name_one_column(self, tmp_path, save_output, save_parameters, column_type):
        columns_parameters = [
            {
                "type": column_type
            }
    ...
    def test_one_value(self, tmp_path, save_output, save_parameters):
        ...
        df = call_dataset_generator(tmp_path, columns_parameters, 500, save_output, save_parameters)
    ...
    def test_text_column_categories(self, tmp_path, save_output, save_parameters, n_categories):
        columns_parameters = [
            {
                "type": "text",
    ...
    def test_text_column_range(self, tmp_path, save_output, save_parameters, range_min, range_max):
        columns_parameters = [
            {
                "type": "text",
    ...
    @pytest.mark.parametrize("range_min", [2, 4, 8])
    @pytest.mark.parametrize("range_max", [12, 14, 18])
    def test_text_column_word_range(self, tmp_path, save_output, save_parameters, range_min, range_max):
        columns_parameters = [
            {
                "type": "text",
                "word_range": (range_min, range_max)
            }
        ]
        df = call_dataset_generator(tmp_path, columns_parameters, 500, save_output, save_parameters)
    ...
    def test_text_column_default_categories(self, tmp_path, save_output, save_parameters):
        columns_parameters = [
            {
                "type": "text",
                "range": (1, 2),
    ...
    def test_text_column_default_word_range(self, tmp_path, save_output, save_parameters):
        columns_parameters = [
            {
                "type": "text",
                "n_categories": 2,
    ...
    @pytest.mark.parametrize("range_min", [2, 4, 8])
    @pytest.mark.parametrize("range_max", [12, 14, 18])
    def test_number_column_float_range(self, tmp_path, save_output, save_parameters, range_min, range_max):
        columns_parameters = [
            {
                "type": "number",
                "float": True,
    ...
    @pytest.mark.parametrize("range_min", [2, 4, 8])
    @pytest.mark.parametrize("range_max", [12, 14, 18])
    def test_number_column_int_range(self, tmp_path, save_output, save_parameters, range_min, range_max):
        columns_parameters = [
            {
                "type": "number",
                "float": False,
