...
def evaluate(df_train, param, prediction_length):
    ...
    univariate_params = {
        "feedforward": params.FeedForwardParams(
    ...
    if df_train.shape[0] <= 1000:
        univariate_params["deepar"] = params.DeepARParams(
            num_cells=20, num_layers=2, epochs=1, context_length=None
        )
    ...
    m = actableai.timeseries.models.AAITimeseriesForecaster(
        ...
        multivariate_model_params=[multivariate_params[param]]
        if target_dim > 1
        else None,
    )
...
for mp in multivariate_params:
    ...
    for df_name in l_data:
        ...
        for i in range(repeat_test):
            ...
            dict_score = {
                your_key: df_score["agg_metrics"][your_key] for your_key in need
            }
...
style1 = new_score_df.style.apply(
    highlight_max, prev_score_df=prev_score_df, props="color:green"
).render()
